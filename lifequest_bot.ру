...
COMPANY_QUESTS = [
    ...
    {"key": "cringiest_outing", "label": "🙈 Самая кринжовая компания",
     "text": "Ваша задача — устроить максимально неловкий совместный выход. Каждый получает от остальных новый образ, в котором должен прийти. Затем отправляйтесь в обычное место и проведите там час, соблюдая придуманный вами сценарий. Финал: сделайте одну общую фотографию, которую вы никогда не решитесь поставить на аватарку."},
    {"key": "never_done_before_quest", "label": "🚪 Квест «Мы никогда так не делали»",
     "text": "Соберите список из 20 вещей, которые никто из вашей компании никогда не делал вместе. Никаких очевидных вариантов вроде «сходить в кино». Перемешайте список и вытяните три задания."},
    {"key": "blind_portrait", "label": "🎨 Портрет вслепую",
     "text": "Каждый рисует другого участника с закрытыми глазами. В конце устройте выставку получившихся шедевров."},
    {"key": "movie_scene_remake", "label": "🎬 Пересъёмка сцены",
     "text": "Выберите знаменитую сцену из фильма и переснимите её своими силами, с реквизитом из того, что под рукой."},
    {"key": "forbidden_common_words", "label": "🗣️ День без обычных фраз",
     "text": "Договоритесь о 5 запрещённых словах на вечер («да», «нет», «привет») — их нужно заменять на что-то другое."},
    {"key": "blindfolded_cooking", "label": "🍕 Кухня вслепую",
     "text": "Приготовьте одно блюдо всей компанией — половина участников с завязанными глазами выполняет команды зрячих."},
    {"key": "escape_room_by_friend", "label": "🗝️ Побег из комнаты за час",
     "text": "Один человек готовит самодельный квест-эскейп для остальных, пока они не видят."},
    {"key": "open_mic_night", "label": "🎤 Открытый микрофон",
     "text": "Каждый выступает с чем угодно 2 минуты: стих, шутка, факт, песня. Аплодисменты обязательны."},
    {"key": "blind_gift_swap", "label": "🎁 Подарок вслепую",
     "text": "Каждый заворачивает нелепую вещь из дома в подарочную бумагу. Обменяйтесь подарками не глядя."},
    {"key": "blind_tasting_company", "label": "🍳 Слепая дегустация компании",
     "text": "Приготовьте несколько блюд и устройте дегустацию с завязанными глазами — угадывайте, кто что готовил."},
    {"key": "round_robin_drawing", "label": "🎨 Круговой рисунок",
     "text": "Каждый рисует часть общей картины 2 минуты, потом передаёт следующему, не объясняя замысел."},
    {"key": "old_photo_journey", "label": "🧳 Путешествие по фотографиям",
     "text": "Каждый показывает одну старую фотографию (не связанную с компанией) и рассказывает историю за ней."},
    {"key": "vlog_of_the_day", "label": "🎬 Дневник одного дня на камеру",
     "text": "Всей компанией снимайте короткие видео-заметки в течение дня, вечером смонтируйте общий влог."},
    {"key": "company_swaps_professions", "label": "🎭 Компания меняет профессии",
     "text": "Каждый на вечер получает случайную новую профессию (космонавт, детектив, пекарь) и должен вести себя и говорить исключительно в этом образе, пока остальные не угадают, кто есть кто."},
    {"key": "radio_play", "label": "🎙️ Радиоспектакль",
     "text": "Придумайте короткую историю и разыграйте её как радиоспектакль — только голосом и звуками, без картинки. Запишите на диктофон и переслушайте."},
    {"key": "portrait_by_description", "label": "🎨 Портрет по описанию",
     "text": "Один описывает внешность и характер отсутствующего общего знакомого, остальные рисуют его по описанию, не видя фото. Сравните рисунки с оригиналом."},
    {"key": "company_from_the_past", "label": "🕰️ Компания из прошлого века",
     "text": "Выберите десятилетие и весь вечер ведите себя, говорите и развлекайтесь так, будто оказались именно там — без телефонов и современных слов."},
    {"key": "thought_narrator", "label": "🎤 Диктор чужих мыслей",
     "text": "Час каждый должен вслух озвучивать, что, по его мнению, думает другой человек в этот момент — сразу после того, как тот что-то сделал или сказал."},
    {"key": "story_from_photos", "label": "🧩 Собери историю по фото",
     "text": "У каждого в галерее есть случайное старое фото. Соберите из пяти таких фото одну общую вымышленную историю и расскажите её по кругу."},
    {"key": "one_prop_theater", "label": "🎭 Театр одного реквизита",
     "text": "Выберите один случайный предмет. По очереди разыгрывайте с ним сценки — как будто это что угодно, кроме того, чем он является на самом деле."},
    {"key": "false_legend_map", "label": "🗺️ Легенда компании на карте",
     "text": "Отметьте на карте точки, которые сегодня посетите, и придумайте для каждой ложную «историческую» легенду — расскажите её друг другу на месте."},
    {"key": "voiceover_documentary", "label": "📻 Голос за кадром",
     "text": "Пока один молча выполняет обычные действия (наливает чай, ищет ключи), остальные по очереди озвучивают это как драматический документальный фильм."},
    {"key": "thread_of_story", "label": "🧵 Нить-история",
     "text": "Разматывая по кругу нитку или верёвку, каждый добавляет одно предложение к общей истории компании — в конце смотрите, какой длины получилась нить."},
]
...
PAIR_QUESTS = [
    ...
    {"key": "embarrassing_date", "label": "🙈 Свидание, за которое вам будет стыдно",
     "text": "Ваша задача — специально устроить максимально кринжовое свидание. Выберите нелепые наряды, придумайте друг другу ужасные комплименты, закажите самые странные блюда и сделайте фотографию, которую вы никогда не стали бы выкладывать. Главное правило: нельзя отменять идею только потому, что стало немного неловко."},
    {"key": "five_star_restaurant", "label": "⭐ Ресторан пяти звёзд",
     "text": "Возьмите обычный домашний ужин и превратите его в ресторан высокой кухни. Придумайте название ресторана, распечатайте или напишите меню, красиво сервируйте стол, включите подходящую музыку и придумайте каждому блюду название. А потом один из вас должен провести другому полноценную дегустацию с комментариями шеф-повара."},
    {"key": "script_for_the_day", "label": "🕵️ Один день под чужим сценарием",
     "text": "Каждый утром пишет для партнёра пять действий, которые тот должен выполнить в течение дня."},
]
